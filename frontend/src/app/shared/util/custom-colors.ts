...
export class CustomColors {
  static primary(tone: Tone, transparency: number = 1): string {
    return CustomColors.getCssColorVariable("--vpms-primary-palette-" + tone.toString(), transparency);
  }

  static accent(tone: Tone, transparency: number = 1): string {
    return CustomColors.getCssColorVariable("--vpms-accent-palette-" + tone.toString(), transparency);
  }

  static grayscale(tone: Tone, transparency: number = 1): string {
    return CustomColors.getCssColorVariable("--vpms-grayscale-" + tone.toString(), transparency);
  }
